...
namespace Fotosteam.Service.Controller
{
    ...
    public class ControllerBase : ApiController
    {
        ...
        protected internal bool IsUserAuthorized
        {
            get
            {
        ...
        protected void LogExceptionAndSetResult<T>(Exception ex, Result<T> result, bool sendEmail = true)
        {
            ...
            if (sendEmail)
                comController.Trello(new Message
                {
                    Title = "Auto exception",
                    SenderName = "fotosteam",
                    SenderEmail = "info@fotosteam.com",
                    Body = FormatException(ex)
                });
        }
        ...
        internal int GetMemberId()
        {
            ...
            return
                DataRepository.Queryable<Member>().Where(m => m.AspNetUserId == id).Select(m => m.Id).SingleOrDefault(
                    );
        }

        internal static string GetUrlRoot()
        {
        ...
        protected internal void ClearSensitiveData(Member member)
        {
            if (member == null)
                return;
